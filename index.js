...
function findLastEvenNumber(arr) {
	...
	let findLastElement = arr.findLast((element) => element % 2 === 0);

	if (findLastElement !== undefined) {
...
function getPositiveArrayValues(arr) {
	...
	const positiveArrayValues = arr2.filter((value) => value > 0);
...
function sumAndPrint(arr) {
	...
	const initValue = 0;
	const sum = arr.reduce((accumulator, currentValue) => accumulator + currentValue, initValue)
	arr.forEach((element) => {
		console.log(element);
	});
	return sum;
...
function flattenSquareAndFilter(arr, threshold) {
	...
	return arr.flat().map((x) => x ** 2).filter((value) => value > threshold);
}
...
function getLastElementsSumAndPrint(arr, n) {
	...
	let sum = lastElements.reduceRight((acc, curr) => {
		return acc + curr;
	}, 0);
...
function findLongestWord(arr) {
	...
	let longestWord = arr.reduce((prev, curr) => {
...
function capitalizeWords(arr) {
	...
	let capitalizedArray = arr.map(function (word) {
...
function calculateTotalPrice(arr) {
	...
	let totalPrice = arr.reduce(function (sum, product) {
...
function findLastNegativeNumberIndex(arr) {
	...
	let lastIndex = arr.findLastIndex(function (num) {
		return num < 0;
	});
